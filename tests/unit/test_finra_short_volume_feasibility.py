...
from scripts.diagnose_finra_short_volume_training import render_markdown
from us_intraday_lab.finra_short_volume_feasibility import (
    FAMILIES,
    run_diagnostic,
    score_family,
    specifications,
...
def test_abnormal_short_flow_reversal_rewards_high_flow_and_low_prior_return() -> None:
    frame = pd.DataFrame(
        {
            "session_date": [date(2022, 1, 3)] * 3,
            "bar_idx": [2] * 3,
            "short_ratio_z20": [2.0, 0.0, -2.0],
            "source_return": [-0.03, 0.0, 0.03],
        }
    )

    score = score_family(frame, "abnormal_high_short_flow_reversal")
...
def _write_fixture(tmp_path: Path) -> tuple[Path, Path]:
    ...
    for session in (date(2022, 1, 3), date(2022, 1, 4)):
            ...
            features.append(
                {
                    ...
                    "short_ratio_z20": float(symbol_index - 1),
                    "source_return": (symbol_index - 1) * 0.02,
                    "short_ratio_change_1": symbol_index * 0.01,
                    "short_exempt_ratio": symbol_index * 0.001,
                    "total_volume_cross_section_pct": (symbol_index + 1) / 3,
                }
            )
...
def test_rejects_hash_mismatch_and_nontraining_rows(tmp_path: Path) -> None:
    events, features = _write_fixture(tmp_path)
    with pytest.raises(RuntimeError, match="FINRA_DIAGNOSTIC_INPUT_HASH_MISMATCH"):
        run_diagnostic(
            events_path=events,
            features_path=features,
...
def test_markdown_preserves_research_only_boundary() -> None:
    markdown = render_markdown(
        {
            "status": "COMPLETE",
            "decision": "ABANDON_FINRA_SHORT_VOLUME_NO_VERSION_CREATED",
            "event_rows": 10,
            "covered_event_rows": 9,
            "cells_completed": 400,
            "retained_cells": 0,
            "retained_families": [],
            "event_sha256": "a" * 64,
