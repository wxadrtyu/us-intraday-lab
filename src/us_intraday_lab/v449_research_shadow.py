"""Pure, brokerless prospective evaluator for the frozen v449 ensemble."""
...
def _component_features(
    ...
    recent_base = _value(buckets, session, symbol, max(0, decision - 6), "close")
...
def _component_signal(buckets: pd.DataFrame, session: date) -> str | None:
    ...
    for symbol in ASSETS:
        features = _component_features(buckets, session, symbol, COMPONENT_DECISION)
    ...
    return selected if scores[selected] >= COMPONENT_THRESHOLD else None
...
def _component_raw_return(
    ...
    *,
    ...
    if not all(np.isfinite((entry, exit_price, spy_entry, spy_exit))) or min(entry, spy_entry) <= 0:
        if strict:
            raise ValueError("v449 component is missing an exact entry or exit bar")
        return 0.0, 0.0, None
    return exit_price / entry - 1.0 - cost, spy_exit / spy_entry - 1.0, symbol


def evaluate_v449_shadow_session(
    bars: pd.DataFrame, *, session_date: date
) -> V449ShadowObservation:
