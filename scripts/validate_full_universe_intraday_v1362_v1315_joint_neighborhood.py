"""v1362 joint neighborhood for the frozen v1315 reversal substitution."""
...
def main() -> None:
    ...
    ids = {
        "reversal": str(definition["reversal_component_id"]),
        "continuation": str(definition["continuation_component_id"]),
    }
    records = {
        name: campaign._component_record(args.component_dir, candidate_id)
        for name, candidate_id in ids.items()
    }
    component_dev = {
        name: campaign._component_streams(development, record)
        for name, record in records.items()
    }
    component_hist = {
        name: campaign._component_streams(historical, record)[0]
        for name, record in records.items()
    }
    ...
    cells = []
    for total_weight, reversal_share, quantile in itertools.product(
        (0.01, 0.02, 0.03), (0.75, 0.90, 1.0), (0.50, 0.60, 0.65, 0.70)
    ):
        ...
        streams = tuple(
            campaign._blend(
                anchor,
                campaign._route(reversal, dev_allowed),
                campaign._route(continuation, dev_allowed),
                total_weight=total_weight,
                v247_share=reversal_share,
                allowed=dev_allowed,
            )
            for anchor, reversal, continuation in zip(
                anchor_dev,
                component_dev["reversal"],
                component_dev["continuation"],
                strict=True,
            )
        )
        ...
        historical_stream = campaign._blend(
            anchor_hist,
            campaign._route(component_hist["reversal"], hist_allowed),
            campaign._route(component_hist["continuation"], hist_allowed),
            total_weight=total_weight,
            v247_share=reversal_share,
        ...
        folds = [
            metrics(
                streams[0].values[index],
                streams[0].benchmark[index],
                streams[0].active[index],
            )
            for index in np.array_split(
                np.flatnonzero(development.masks()["development_all"]), 5
            )
        ]
        gates = {
        ...
        cells.append(
            {
                ...
                "reversal_component_share": reversal_share,
                "state_quantile": quantile,
                "oos_annualized_return": standard["development_oos_2024_2025"][
                    "annualized_return"
                ],
                "oos_max_drawdown": standard["development_oos_2024_2025"][
                    "max_drawdown"
                ],
                "oos_information_ratio": standard["development_oos_2024_2025"][
                    "information_ratio"
                ],
                "cost_18bp_annualized_return": observations[1][
                    "development_oos_2024_2025"
                ]["annualized_return"],
                "delay_5min_annualized_return": observations[2][
                    "development_oos_2024_2025"
                ]["annualized_return"],
                "historical_annualized_return": historical_obs["annualized_return"],
                "historical_max_drawdown": historical_obs["max_drawdown"],
                "consumed_2026q1_total_return": standard["consumed_2026q1"][
                    "total_return"
                ],
                "consumed_2026_total_return": standard["consumed_2026_all"][
                    "total_return"
                ],
                "gates": gates,
