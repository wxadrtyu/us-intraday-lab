"""Start the explicitly authorized v247 brokerless research shadow."""
...
def main() -> None:
    ...
    if _json_sha256(selection) != selection_hash:
        raise ValueError("v247 exception selection manifest hash mismatch")
    proposal_hash = _json_sha256(proposal)
    if selection["proposal_sha256"] != proposal_hash:
        raise ValueError("v247 exception proposal hash does not match selection")
    ...
    if selection["promotion_status"] != "USER_AUTHORIZED_RESEARCH_SHADOW_EXCEPTION":
        raise ValueError("v247 candidate lacks an explicit research-shadow exception")
    if authority != {
        "actor": "user",
        "date": "2026-08-17",
        ...
        raise ValueError("v247 exception authority is invalid")
    required_passes = (
    ...
    if not all(gates[name] for name in required_passes):
        raise ValueError("v247 passed gates are incomplete")
    if (
        gates["inherited_v45_factory_null_passed"]
        or gates["global_bonferroni_passed"]
        or gates["all_hard_gates_passed"]
    ):
        raise ValueError("v247 inherited failed-gate state must remain explicit")
    forward = proposal["forward_contract"]
    if args.start_session_not_before <= date.fromisoformat(forward["must_start_after"]):
        raise ValueError("v247 research shadow must start after all consumed data")
    if proposal["scope"]["order_route"] != "FORBIDDEN":
        raise ValueError("v247 research shadow cannot expose an order route")
