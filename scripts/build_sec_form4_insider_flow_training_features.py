"""Build causal SEC Form 4 event features for the frozen training sample."""
...
from us_intraday_lab.data.sec_form4_insider_flow import build_event_features
...
def main() -> int:
    ...
    try:
        if arguments.output.exists():
            if arguments.output.read_bytes() != temporary.read_bytes():
                raise RuntimeError("SEC_FORM4_FEATURE_IMMUTABLE_COLLISION")
        else:
            temporary.replace(arguments.output)
    finally:
        temporary.unlink(missing_ok=True)
    inventory = filings[["symbol", "accession"]].drop_duplicates().groupby(
    ...
    summary = {
        ...
        "training_only": True,
        "rows": len(result),
        "qualifying_filing_events": len(filings),
        "issuers_with_three_qualifying_filings": int(inventory.ge(3).sum()),
        "filing_years": sorted(
