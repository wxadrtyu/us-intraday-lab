"""Conditional protective exit with at most one causal, fully costed re-entry."""
...
PROPOSAL = Path(__file__).resolve().parents[1] / (
    "research/proposals/full_universe_intraday_v2866_v2965_one_reentry/proposal.json"
)
MARKET_CONFIRMATION = -0.0025
PROFIT_ACTIVATION = 0.015
REENTRY_RECOVERY = 0.005
...
def stopped_raw(
    ...
    for bar in range(int(first_monitor.min()), fixed_exit):
        ...
        complete = (
            ...
            & (cube.last[:, bar, 0] >= bar * 5 + 4 - cube.boundary_tolerance)
        )
        close, market_close = cube.closes[rows, bar, safe_asset], cube.closes[:, bar, 0]
        observed = (
            eligible
            & complete
            & np.isfinite(close)
        ...
        hard = (close / entry_price - 1.0 <= -hard_stop) & (
            market_close / market_entry - 1.0 <= MARKET_CONFIRMATION
        )
    ...
    first_leg = (
        cube.opens[rows[reentered], exit_bar[reentered], safe_asset[reentered]]
        / entry_price[reentered]
    )
    second_leg = (
...
def build_streams(cube, development, record, models, definition):
    streams, valids, exits = ORIGINAL_BUILD_STREAMS(cube, development, record, models, definition)
    output = []
