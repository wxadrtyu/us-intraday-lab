...
def _fit_gate(cube, transfer_standard, base_transfer, factors, quantile, alpha):
    ...
    train = (
        cube.masks()["train_2022_2023"]
        & base_transfer
        & transfer_standard.active
    ...
    coefficients = np.linalg.solve(
        standardized.T @ standardized + alpha * np.eye(len(factors)), standardized.T @ target
    )
...
def _route(parent_streams, base_modern, allow_transfer):
    ...
    active_fallback = (~base_modern) & (~allow_transfer) & (FALLBACK_PARENT is not None)
    output = []
    fallback_streams = (
        parent_streams[FALLBACK_PARENT] if FALLBACK_PARENT is not None else (None,) * 3
    )
    for modern, transfer, fallback in zip(
        parent_streams[MODERN_PARENT],
        parent_streams[TRANSFER_PARENT],
        fallback_streams,
        strict=True,
    ):
        fallback_values = fallback.values if fallback is not None else 0.0
        fallback_benchmark = fallback.benchmark if fallback is not None else 0.0
        fallback_active = (
            active_fallback & fallback.active if fallback is not None else active_fallback
        )
        fallback_trades = fallback.component_trades if fallback is not None else 0
        values = np.where(
            base_modern,
            modern.values,
            np.where(
                active_transfer, transfer.values, np.where(active_fallback, fallback_values, 0.0)
            ),
        )
        benchmark = np.where(
            base_modern,
            ...
            np.where(
                active_transfer,
                transfer.benchmark,
                np.where(active_fallback, fallback_benchmark, 0.0),
            ),
        )
        active = np.where(
            base_modern, modern.active, np.where(active_transfer, transfer.active, fallback_active)
        )
        trades = np.where(
            base_modern,
            ...
            np.where(
                active_transfer,
                transfer.component_trades,
                np.where(active_fallback, fallback_trades, 0),
            ),
        )
...
def main() -> None:
    ...
    required = (
        tuple(dict.fromkeys((MODERN_PARENT, TRANSFER_PARENT, FALLBACK_PARENT)))
        if FALLBACK_PARENT is not None
        else (MODERN_PARENT, TRANSFER_PARENT)
    )
    ...
    models = {
        item: v39._models(development, [parent_map[item]["definition"]["strategy"]])[0]
        for item in required
    }
    dev_parents = {
        item: prior._parent_streams(development, parent_map[item], models[item])
        for item in required
    }
    hist_parents = {
        item: prior._parent_streams(historical, parent_map[item], models[item]) for item in required
    }
    ...
    for cell in cells:
        ...
        definition = {
            ...
            "mechanism": MECHANISM,
            "base_candidate": "lev-v4423-5298677326af2b2b",
        ...
        records.append(
            {
                ...
                "historical_scenarios": {
                    "standard": historical_obs[0],
                    "cost_18bp": historical_obs[1],
                    "delay_5min_9bp": historical_obs[2],
                },
