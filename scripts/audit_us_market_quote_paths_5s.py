...
from scripts.audit_us_market_trade_paths_30s import expected_stems, sha256_file
...
def main() -> None:
    ...
    staging = args.root / "data/staging/alpaca_sip_quote_path_5s_v1"
    ...
    for (month, bar_idx, stem), expected_rows in expected.items():
        ...
        canonical.append(manifest_path)
        ...
        if (manifest.get("provider") != "alpaca" or manifest.get("feed") != "sip"
                or manifest.get("window_seconds") != 5
                or manifest.get("source_type") != "historical_quotes"):
    ...
    extras = sorted(str(path) for path in actual_manifests.difference(canonical_set))
    ...
    bad_window_rows = int(pd.to_numeric(combined["window_seconds"], errors="coerce").ne(5).sum())
    ...
    passed = (
        not any((missing_artifacts, hash_failures, row_count_failures, contract_failures))
        and duplicate_keys == unexpected_keys == missing_keys == 0
        and bad_window_rows == bad_provider_rows == bad_feed_rows == bad_session_rows == 0
    )
    cache = args.root / "research/cache/us_market_event_quote_path_5s_v1.parquet"
    ...
    result = {
        ...
        "protocol_id": "us-market-event-quote-path-5s-v1",
        "start": args.start.isoformat(), "end": args.end.isoformat(),
        "expected_event_rows": len(events), "actual_event_rows": len(combined),
        "available_rows": int(available.sum()), "missing_quote_rows": int((~available).sum()),
        "availability_rate": float(available.mean()), "expected_shards": len(expected),
        "canonical_shards": len(canonical) - len(missing_artifacts),
        "extra_probe_shards_excluded": len(extras), "missing_artifacts": missing_artifacts,
        "hash_failures": hash_failures, "row_count_failures": row_count_failures,
        "contract_failures": contract_failures, "duplicate_key_rows": duplicate_keys,
        "missing_event_keys": missing_keys, "unexpected_event_keys": unexpected_keys,
        "bad_window_rows": bad_window_rows, "bad_provider_rows": bad_provider_rows,
        "bad_feed_rows": bad_feed_rows, "bad_session_rows": bad_session_rows,
        "causal_boundary_evidence": "fixed [decision_timestamp-5s, decision_timestamp) request and strict raw timestamp < cutoff; no progressive widening",
    ...
    args.report.with_suffix(".md").write_text(
        "# US market SIP fixed-five-second quote-path audit\n\n"
        f"- Status: `{result['status']}`\n"
        f"- Event rows: {result['actual_event_rows']:,} / {result['expected_event_rows']:,}\n"
        f"- Quote availability: {result['available_rows']:,} ({result['availability_rate']:.4%})\n"
        f"- Explicit no-quote rows: {result['missing_quote_rows']:,}\n"
