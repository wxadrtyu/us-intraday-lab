"""Bounded multi-factor VWAP-reclaim event campaign."""
...
EXIT = 72
ASSETS = np.array((3, 4))
PRIOR20_CEILINGS = (0.0, 0.05)
CURRENT_FLOORS = (0.0, 0.005)
VWAP_CROSS_BANDS = ((-0.002, 0.0), (0.0, 0.0), (-0.002, 0.002))
VOLUME_ACCELERATION_CEILINGS = (0.0, 0.25)
PATH_EFFICIENCY_FLOORS = (0.10, 0.20)
TARGETS = (0.25, 0.30, 0.35)
LOOKBACK = 20
WEAK_MARKET_2026_MIN_TOTAL_RETURN = 0.05
CANDIDATE_PREFIX = "lev-v50w-"
SELECTION_CONTRACT = (
    "the five-factor VWAP-reclaim event and bounded family were declared before "
    "historical and 2026 diagnostics"
)


def _selected(cube: v34.Cube, definition: dict):
    ...
    for decision in HORIZONS:
        ...
        vwap = factors["vwap_distance"][:, ASSETS]
        volume = factors["volume_acceleration"][:, ASSETS]
        path = factors["path_efficiency"][:, ASSETS]
        ...
        eligible = (
            finite
            & (current >= float(definition["current_floor"]))
            & (vwap >= float(definition["vwap_current_floor"]))
            & (volume <= float(definition["volume_acceleration_ceiling"]))
            & (path >= float(definition["path_efficiency_floor"]))
            & (prior20 <= float(definition["prior20_ceiling"]))
        )
        if previous_vwap is None:
            eligible[:] = False
        else:
            eligible &= np.isfinite(previous_vwap) & (
                previous_vwap <= float(definition["vwap_previous_ceiling"])
            )
        score = current - prior20 + 0.25 * path
        score = np.where(eligible, score, -np.inf)
        local = np.argmax(score, axis=1)
        best = score[cube.rows, local]
        trigger = (selected < 0) & np.isfinite(best)
        selected[trigger] = ASSETS[local[trigger]]
        decision_bar[trigger] = decision
        previous_vwap = vwap
    return selected, decision_bar


def _stream(cube: v34.Cube, definition: dict, cost: float, delay: int):
    selected, decision = _selected(cube, definition)
    safe_asset = np.maximum(selected, 0)
    entry = decision + 1 + delay
    safe_entry = np.maximum(entry, 0)
    active = (selected >= 0) & (entry < EXIT)
    active &= (
        cube.first[cube.rows, safe_entry, safe_asset] <= safe_entry * 5 + cube.boundary_tolerance
    )
    active &= cube.first[cube.rows, EXIT, safe_asset] <= EXIT * 5 + cube.boundary_tolerance
    active &= np.isfinite(cube.opens[cube.rows, safe_entry, safe_asset])
    active &= np.isfinite(cube.opens[cube.rows, EXIT, safe_asset])
    active &= np.isfinite(cube.opens[cube.rows, safe_entry, 0])
    active &= np.isfinite(cube.opens[:, EXIT, 0])
    ...
    values[active] = (
        cube.opens[active, EXIT, safe_asset[active]]
    ...
    benchmark[active] = (
        cube.opens[active, EXIT, 0] / cube.opens[cube.rows[active], safe_entry[active], 0] - 1.0
    )
...
def _scaled(cube: v34.Cube, definition: dict):
    ...
    exposure = v42._exposure(raw[0].values, LOOKBACK, float(definition["target_volatility"]), 0.0)
...
def main() -> None:
    ...
    candidates = []
    planned = 0
    for prior20, current, band, volume, path, target in itertools.product(
        PRIOR20_CEILINGS,
        CURRENT_FLOORS,
        VWAP_CROSS_BANDS,
        VOLUME_ACCELERATION_CEILINGS,
        PATH_EFFICIENCY_FLOORS,
        TARGETS,
    ):
        planned += 1
        definition = {
            ...
            "vwap_previous_ceiling": band[0],
            "vwap_current_floor": band[1],
            "volume_acceleration_ceiling": volume,
            "path_efficiency_floor": path,
        ...
        candidates.append(
            (
                ...
                v12._identity(definition, CANDIDATE_PREFIX),
                definition,
                streams,
            )
        )
    ...
    # Freeze the 144-cell event family before historical and 2026 diagnostics.
