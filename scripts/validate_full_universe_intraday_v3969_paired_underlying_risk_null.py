"""Family-wise risk-timing null for paired-underlying confirmation."""
...
import evaluate_full_universe_intraday_v3869_v3968_paired_underlying_risk as paired
import numpy as np

PROPOSAL = Path(__file__).resolve().parents[1] / (
    "research/proposals/full_universe_intraday_v3969_paired_underlying_risk_null/proposal.json"
)
...
def risk_statistic(candidate: tuple[np.ndarray, ...], matched: tuple[np.ndarray, ...]) -> float:
    ...
    for values, baseline in zip(candidate, matched, strict=True):
        mdd_reduction = 1.0 - max_drawdown(values) / max_drawdown(baseline)
        tail_reduction = 1.0 - base.risk.tail_loss(values) / base.risk.tail_loss(baseline)
...
def main() -> None:
    ...
    if len(eligible) != int(proposal["eligible_family_size"]):
        raise RuntimeError("PRE_NULL_FAMILY_SIZE_CHANGED")
    frozen = next(item for item in eligible if item["candidate_id"] == proposal["frozen_candidate"])
    ...
    baseline = tuple(base.risk.add(left, right) for left, right in base.risk.baseline_parts(cube, cube))
    oos = cube.masks()["development_oos_2024_2025"]
    candidates = []
    base.stopped_raw = paired.stopped_raw
    for item in eligible:
        streams, valids, _ = paired.build_streams(cube, cube, component, models, item["definition"])
        candidate_values = tuple(stream.values[oos] for stream in streams)
        matched_values = tuple(
            baseline[index].values[oos] * valids[index][oos] for index in range(3)
        )
        candidates.append(
            {
                "candidate_id": item["candidate_id"],
                "matched": matched_values,
                "delta": tuple(values - matched for values, matched in zip(candidate_values, matched_values, strict=True)),
    ...
    for _ in range(int(proposal["repetitions"])):
        ...
        permutation_max.append(
            max(
                risk_statistic(
                    tuple(matched + delta[permutation] for matched, delta in zip(item["matched"], item["delta"], strict=True)),
                    item["matched"],
                )
        ...
        shift_max.append(
            max(
                risk_statistic(
                    tuple(matched + np.roll(delta, shift) for matched, delta in zip(item["matched"], item["delta"], strict=True)),
                    item["matched"],
                )
                for item in candidates
            )
    ...
    payload = {
        ...
        "historical_15pct_gate_passed": False,
        "cumulative_bonferroni_passed": False,
        "classification": "RISK_MILESTONE_HISTORICAL_EXCEPTION_REVIEW" if passed else "REJECTED_RISK_NULL",
        "admitted": False,
        ...
        "contract": {
            ...
            "dependency_sha256": {
                ...
                Path(paired.__file__).name: sha(Path(paired.__file__)),
            },
        },
    }
