...
def main() -> None:
    ...
    staging = args.root / "data/staging/alpaca_sip_trade_features_1s_v1"
    ...
    for (month, bar_idx, stem), expected_rows in expected.items():
        ...
        if manifest.get("content_sha256") != sha256_file(parquet_path):
            ...
            continue
        if manifest.get("provider") != "alpaca" or manifest.get("feed") != "sip" or manifest.get("window_seconds") != 1 or manifest.get("source_type") != "historical_trades":
    ...
    bad_window_rows = int(pd.to_numeric(combined["window_seconds"], errors="coerce").ne(1).sum())
    ...
    passed = not any((missing_artifacts, hash_failures, row_count_failures, contract_failures)) and duplicate_keys == 0 and unexpected_keys == 0 and missing_keys == 0 and bad_window_rows == 0 and bad_provider_rows == 0 and bad_feed_rows == 0 and bad_session_rows == 0

    cache = args.root / "research/cache/us_market_event_trade_features_1s_v1.parquet"
    ...
    result = {
        ...
        "protocol_id": "us-market-event-trades-1s-v1",
        "start": args.start.isoformat(),
        "end": args.end.isoformat(),
        "expected_event_rows": len(events),
        "actual_event_rows": len(combined),
        "available_rows": int(available.sum()),
        "missing_trade_rows": int((~available).sum()),
        ...
        "bad_window_rows": bad_window_rows,
        "bad_provider_rows": bad_provider_rows,
        "bad_feed_rows": bad_feed_rows,
        "bad_session_rows": bad_session_rows,
        "causal_boundary_evidence": "fixed [decision_timestamp-1s, decision_timestamp) request plus strict raw timestamp < cutoff filter in frozen acquisition code; raw trade timestamps intentionally not retained in aggregate cache",
    ...
    args.report.with_suffix(".md").write_text(
        ...
        f"- Noncanonical one-day probe shards excluded: {result['extra_probe_shards_excluded']:,}\n"
