"""Build causal SEC beneficial-ownership features for the frozen training sample."""
...
from us_intraday_lab.data.sec_beneficial_ownership import build_event_features
...
def build_feature_cache(
    ...
    """Atomically publish a deterministic feature cache and its audit summary."""
    ...
    try:
        if output.exists():
            if output.read_bytes() != temporary.read_bytes():
                raise RuntimeError("SEC_BENEFICIAL_FEATURE_IMMUTABLE_COLLISION")
        else:
            temporary.replace(output)
    finally:
        temporary.unlink(missing_ok=True)
    issuer_filings = filings[["cik", "accession_number"]].drop_duplicates()
    issuer_counts = issuer_filings.groupby("cik", observed=True).size()
    return {
        "status": "COMPLETE",
        "training_only": True,
        "rows": len(result),
        "qualifying_symbol_filings": len(
            filings[["symbol", "accession_number"]].drop_duplicates()
        ),
        "unique_issuer_filings": len(issuer_filings),
        "unique_cik_issuers": int(filings["cik"].nunique()),
        "issuers_with_three_qualifying_filings": int(issuer_counts.ge(3).sum()),
        "filing_years": sorted(
...
def main() -> int:
    ...
    summary = build_feature_cache(
        events, filings, identities, arguments.output
    )
