"""v2066: native factory nulls for frozen v1966-v2065 state survivors."""
...
import evaluate_full_universe_intraday_v1966_v2065_transition_campaign as config
import numpy as np
import pandas as pd
...
def _pre_null_candidates(source_dir: Path) -> list[dict]:
    candidates = []
    for version in range(1966, 2066):
...
def main() -> None:
    ...
    if not candidates:
        raise RuntimeError("v2066 requires at least one frozen pre-null candidate")
    config.configure()
    ...
    for candidate in candidates:
        ...
        result = run_null_tests(
            ...
            scoring_config=HoldingRuleScoringConfig(
                scoring_id=f"v2066-{candidate['candidate_id']}-state-null",
    ...
    payload = {
        ...
        "status": "COMPLETE",
        "version": 2066,
        "selection_contract": "v1966-v2065 frontiers froze on 2022-2025 before native null; 2026 remains consumed diagnostic",
