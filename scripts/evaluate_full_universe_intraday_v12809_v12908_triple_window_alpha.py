"""Three non-overlapping intraday residual-alpha sleeves per session."""
...
import evaluate_full_universe_intraday_v12709_v12808_dual_window_alpha as dual
import numpy as np

FIRST_VERSION = 12809
LAST_VERSION = 12908
PRIOR_COMPARISON_CELLS = 321_183
...
class TripleWindowModel:
    ...
    sleeves: tuple[base.Model, base.Model, base.Model]


def _fit(cube, family, schedule_triple):
    sleeves = tuple(
        dual.ORIGINAL_FIT(cube, family, schedule) for schedule in schedule_triple
    )
    return TripleWindowModel(
        family=family,
...
def _sleeve_scores(cube, model):
    return tuple(dual.ORIGINAL_SCORES(cube, sleeve) for sleeve in model.sleeves)
...
def _raw(cube, model, top_k, threshold, cost, delay):
    ...
    sleeve_streams = tuple(
        dual._single_raw(cube, sleeve, top_k, cost, delay)
...
def _configure() -> None:
    ...
    base.MECHANISM = "non_overlapping_triple_window_residual_alpha"
