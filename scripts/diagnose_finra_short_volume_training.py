"""Run the frozen FINRA short-volume training feasibility diagnostic."""
...
from us_intraday_lab.finra_short_volume_feasibility import run_diagnostic
...
def _write_immutable(path: Path, content: bytes) -> None:
    ...
    if path.exists():
        if path.read_bytes() != content:
            raise RuntimeError(f"FINRA_DIAGNOSTIC_IMMUTABLE_COLLISION:{path}")
        return
...
def render_markdown(summary: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# FINRA Short-Volume Training Feasibility",
            "",
            f"- Status: **{summary['status']}**",
            f"- Decision: **{summary['decision']}**",
            f"- Training event rows: {summary['event_rows']:,}",
            f"- Causally covered rows: {summary['covered_event_rows']:,}",
