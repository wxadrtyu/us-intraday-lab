"""Verify the live-frame v798 state calculation against the research cube."""
...
from us_intraday_lab.paper.pool import V798_STATE_STATS, v798_state_score
...
def main() -> None:
    ...
    if len(selected_indexes) != args.sessions:
        raise RuntimeError("V798_PAPER_FACTOR_PARITY_INSUFFICIENT_COMPLETE_SESSIONS")
    ...
    for index in selected_indexes:
        ...
        observed = v798_state_score(bars, session_date=target)
        pieces = []
        for name, (mean, scale, direction) in V798_STATE_STATS.items():
            pieces.append(direction * (float(matrix[name][index]) - mean) / scale)
    ...
    if not passed:
        raise RuntimeError("V798_PAPER_FACTOR_PARITY_FAILED")
