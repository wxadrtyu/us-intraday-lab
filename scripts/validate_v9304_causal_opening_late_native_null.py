"""v9304 native maxT null for causal fixed-opening plus late soft-veto candidates."""
...
import evaluate_full_universe_intraday_v9204_v9303_causal_opening_late_soft_veto as causal
import numpy as np

VALIDATION_VERSION = 9304
REPETITIONS = 500
PERCENTILE = 0.95
SEED = 20260901
...
def _development_route(root: Path, source_path: Path):
    causal._configure()
    campaign = causal.sparse_veto.campaign
    if campaign.base.prior.parent._sha(source_path) != campaign.base.prior.SOURCE_SHA256:
        raise RuntimeError("V9304_SOURCE_HASH_CHANGED")
    ...
    opening = causal._opening_by_late_stream[id(late)]
    return development, factors, late, opening
...
def main() -> None:
    ...
    if selection.get("status") != "COMPLETE" or selection.get("version_range") != [9204, 9303]:
        raise RuntimeError("V9304_SELECTION_NOT_FROZEN_COMPLETE")
    eligible = [item for item in selection["records"] if item["strict_pre_factory_null_pass"]]
    if len(eligible) != 1:
        raise RuntimeError("V9304_ELIGIBLE_CANDIDATE_COUNT_CHANGED")
    development, factors, late, opening = _development_route(args.root, args.source)
    ...
    for item in eligible:
        ...
        model = quality._fit(
            ...
            late,
            late.active,
            causal.sparse_veto.campaign.FACTOR_SETS[definition["factor_set"]],
            float(definition["score_quantile"]),
            float(definition["ridge_alpha"]),
        )
        score = quality._score(factors, model)
        exposures.append(
            np.where(
